...
func handleClientPut(args []string, masterIP string, clientMessage *pb.SdfsMessage, shouldAppend bool) {
	clientMessage.FileOp = pb.FileOp_PUT
	clientMessage.SdfsFilename = strings.ReplaceAll(args[2], "/", "_")
	clientMessage.ShouldAppend = shouldAppend
	...
	if hasRead {
		...
		if replyMessage != nil {
			...
			logger.PrintError("connection with master failed")
		}
	} else {
...
func HandleCommands(input, clientIP, masterIP string, isSdfsDir, shouldAppend bool) {
	// startTime := time.Now()
	...
	if args[0] == "put" && len(args) == 3 {
		handleClientPut(args, masterIP, clientMessage, shouldAppend)
	...
	clientMessage.SdfsFilename = strings.ReplaceAll(clientMessage.SdfsFilename, "/", "_")
...
func Run(master string) {
	...
	for scanner.Scan() {
		input := scanner.Text()
		HandleCommands(input, clientIP, master, false, false)
	}
